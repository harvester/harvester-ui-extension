...
const DEFAULT_VALUE = { imagePreloadOption: { strategy: { type: 'sequential' } }, restoreVM: false };
...
export default {
  ...
  data() {
    ...
    try {
      parseDefaultValue = this.value.value ? JSON.parse(this.value.value) : { ...DEFAULT_VALUE };
    } catch (error) {
      parseDefaultValue = { ...DEFAULT_VALUE };
    }
